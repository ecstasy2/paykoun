...
let logger = new ConsoleLogger('Paykoun/Context');

const HELPERS_PREFIX = '$_';


function PaykounContext(workQueueMgr) {
  this.allWorkers = {};
...
PaykounContext.prototype.defaultQueue = function() {
  if (!this.emitQueue) {
    this.emitQueue = this.workQueueManager.createQueue('Default');
  }
  return this.emitQueue;
};

PaykounContext.prototype.createJob = function(event, params){
  return this.defaultQueue().createJob(event, params)
};
...
PaykounContext.prototype.useHelper = function(name, helper) {
  ...
  assert.ok(_.isString(name), 'helper name should be a string');
  assert.ok(helper, 'you need to provide a helper');
  assert.ok(_.isFunction(helper) || _.isObject(helper), 'helper need to be a function or object');

  if (this.helpers[HELPERS_PREFIX + name]) {
...
PaykounContext.prototype.run = function(done) {
  ...
  this.workQueueManager.on('ready', () => {
    var arg = {
      queues: queues,
...
PaykounContext.prototype.connectWorkQueues = function(params, callback) {
  ...
  vasync.forEachParallel({
    ...
    'inputs': params.queues.concat([this.defaultQueue()])
  }, function (err) {
    callback(err, null);
  });
};
...
PaykounContext.prototype.connectWorkQueue = function (workQueue, callback){
  ...
  const onReady = () => {
    this.workQueues[workQueue.name] = workQueue;
